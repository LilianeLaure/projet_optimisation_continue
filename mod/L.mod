...
s.t. capacity2{ i in I, k in I }: ((sum{j in {1..k-1}} A[i,j]*x[i,j])+(sum{j in {k+1..M}} A[i,j]*x[i,j])) - ((sum{j in {1..k-1}} A[i,j]*z[i,j,k])+(sum{j in {k+1..M}}A[i,j]*x[i,j])) <= b[i]-b[i]*x[i,k];
